...
const ConsumerOwnerUnitDB = new DB("http://localhost:55317/api/consumer_owner_unit");
const accountDB = new DB("http://localhost:55317/api/accounts");
const ReqTypeDB = new DB("http://localhost:55317/api/request_type");
const RFS_DB = new DB("http://localhost:55317/api/request_of_service");
const DFS_DB = new DB("http://localhost:55317/api/disconnect_of_service");
const requestStaffAssignDB = new DB("http://localhost:55317/api/request_staff_assign");
const inspectionDB = new DB("http://localhost:55317/api/inspections");
const staffDB = new DB("http://localhost:55317/api/staffs");
const subStationsDB = new DB("http://localhost:55317/Substations");
const requestsDB = new DB("http://localhost:55317/api/request_of_service");
const billsDB = new DB("http://localhost:55317/api/bills");
...
export default class SingleApproval extends Component {
    ...
    componentWillMount() {
        ...
        requestsDB.findOne(sessionStorage.getItem('requestTypeID'), (data) => this.setState({request: data}))

           console.log("loading all substations")

        subStationsDB.find((data) => this.setState({ substations: data }))
        requestsDB.find((data) => this.setState({ requests: data }))
    ...
    requestAnInspection = () => {
        ...
        inspectionDB.Create({
            ...
            "Status": "onGoing",
            "Date": doc.value,
            "Request_Type_Id": sessionStorage.getItem("requestTypeID"),
   ...
   approveRequest = () => {
    ...
    approveRequestReal = () => {
        ...
        requestsDB.ApproveRequest({ "requestId": 0, "substationId": this.state.substations[index].SubstationId });
